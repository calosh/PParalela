...
int main(int argc, char **argv)
{
	...
	MPI_Status status;
	int my_id, root_process, ierr, i,j, num_rows, num_procs,
	...
	* to determine how many numbers to sum. */
	printf("Ingrese numeros de elementos: ");
	scanf("%i", &num_rows);
	...
	/*Copiar la diagonal en un array*/
	for(i = 0; i < num_rows; i++) {
		array[i] = matrix[i][i];
	}
	...
	* to the root process */
	...
	printf("Suma %li calculada en el proceso root\n", sum);
	...
	* print them, and add them to the grand sum, and print it */
	...
	printf("Suma parcial %li retornada por el proceso %i\n", partial_sum, sender);
	sum += partial_sum;
	}
	printf("La suma total es: %li\n", sum);
	}
	else {
	...
	* storing it in a "local" array, array1. */
	...
	printf("%.16g milisegundos en el proceso %i \n", secs*1000, my_id);
	return 0;
}